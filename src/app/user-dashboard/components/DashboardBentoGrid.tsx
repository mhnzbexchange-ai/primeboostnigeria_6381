...
export default function DashboardBentoGrid() {
  ...
  const fetchStats = async () => {
    ...
    try {
      const { data: wallet } = await supabase
        .from('wallets')
        .select(
          'balance, referral_earnings, pending_referral_withdrawal'
        )
        .eq('user_id', user.id)
        .maybeSingle();
      ...
      setStats({
        ...
        failedOrders,
        referralEarnings: Number(
          wallet?.referral_earnings || 0
        ),
        pendingReferralWithdrawal: Number(
          wallet?.pending_referral_withdrawal || 0
        ),
        thisMonthSpend,
      });
    } catch (err: any) {
  ...
  const handleFundWallet = async () => {
    ...
    if (!user?.email) {
      toast.error(
        'Please log in again before funding your wallet.'
      );
      return;
    }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="card-base card-gradient-bg hover:border-primary/20 transition-all">
          ...
          <div className="mt-3 flex gap-1.5">
            <span className="badge-base status-completed">
        ...
        >
          ...
          {stats.activeOrders > 0 && (
            <div className="mt-3">
              <span className="badge-base status-processing">
                In progress
              </span>
            </div>
          )}
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">
            Failed orders · Refunds pending
          </p>

          <div className="mt-3">
            <button className="text-xs text-red-400 hover:underline font-medium">
              View failed orders →
            </button>
          </div>
        </div>
