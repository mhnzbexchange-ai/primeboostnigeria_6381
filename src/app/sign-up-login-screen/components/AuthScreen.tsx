...
import { Eye, EyeOff, ArrowRight, Star, Loader2, X, Mail, CheckCircle, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 xl:w-2/5 p-12 relative overflow-hidden bg-card border-r border-border">
        ...
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-80 h-80 rounded-full opacity-10 blur-3xl" style={{ background: 'var(--primary)' }} />

        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-12">
            <AppLogo size={40} />
            <span className="font-bold text-lg gold-gradient-text tracking-wide">PrimeBoost Nigeria</span>
          </div>

          <h2 className="text-4xl font-extrabold leading-tight mb-6">
            Grow Your<br />
            <span className="gold-gradient-text">Social Media</span><br />
            Presence Fast.
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            {[
              ...
              { value: '380K+', label: 'Orders Fulfilled' },
              { value: '₦2.4B+', label: 'Value Delivered' },
              { value: '4.9/5', label: 'Customer Rating' },
            ].map((s) => (
              <div key={`auth-stat-${s.label.replace(/\s/g, '-')}`} className="glass-card p-3 rounded-xl">
                <div className="font-extrabold text-xl gold-gradient-text tabular-nums">{s.value}</div>
          ...
          <div className="glass-card p-4 rounded-xl border border-primary/20">
            <div className="flex gap-0.5 mb-2">
              {[1,2,3,4,5].map((s) => <Star key={`auth-star-${s}`} size={12} className="text-primary fill-primary" />)}
            </div>
            <p className="text-sm text-muted-foreground italic mb-3">&ldquo;PrimeBoost took my TikTok from 2K to 45K followers. Now I get paid brand deals!&rdquo;</p>
            <p className="text-xs font-semibold">Chinyere Okonkwo · TikTok Creator</p>
          </div>
        </div>

        <div className="relative z-10">
          <p className="text-xs text-muted-foreground">🇳🇬 Proudly Nigerian · Naira Payments · 24/7 Support</p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-muted-foreground">
              {tab === 'login' ? 'Sign in to your PrimeBoost account' : 'Join Nigeria\'s top promotion platform'}
            </p>
          </div>
          ...
          <div className="flex bg-muted/40 rounded-xl p-1 mb-8">
            {(['login', 'register'] as const).map((t) => (
              <button
                key={`tab-${t}`}
                onClick={() => { setTab(t); setRegisteredEmail(null); }}
                className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${
                  tab === t ? 'gold-gradient-bg text-primary-foreground shadow-gold-sm' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
          ...
          {registeredEmail && tab === 'register' && (
            <div className="glass-card border border-primary/30 rounded-xl p-5 mb-6 text-center">
              ...
              <h3 className="font-bold text-base mb-2">Check Your Email</h3>
              <p className="text-sm text-muted-foreground mb-1">
                We sent a verification link to
              </p>
              <p className="text-sm font-semibold text-primary mb-3">{registeredEmail}</p>
                ...
                <div className="glass-card border border-yellow-500/30 bg-yellow-500/5 rounded-xl p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-yellow-400 mb-1">Email not verified</p>
                      <p className="text-xs text-muted-foreground mb-3">
                        Please verify <span className="text-foreground font-medium">{unverifiedEmail}</span> before signing in. Check your inbox for the verification link.
                      </p>
                      {resendSent ? (
            ...
            <form onSubmit={registerForm.handleSubmit(onRegisterSubmit)} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="input-field"
                    placeholder="Adaeze"
                    {...registerForm.register('firstName', { required: 'First name is required', minLength: { value: 2, message: 'Too short' } })}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className="input-field"
                    placeholder="Chukwu"
                    {...registerForm.register('lastName', { required: 'Last name is required', minLength: { value: 2, message: 'Too short' } })}
                  />
              ...
              <label className="flex items-start gap-2 cursor-pointer">
                <input
                  ...
                  {...registerForm.register('terms', { required: 'You must accept the terms' })}
                />
              ...
              {registerForm.formState.errors.terms && (
                <p className="text-red-400 text-xs">{registerForm.formState.errors.terms.message}</p>
              )}
              ...
              <>
                ...
                <form onSubmit={forgotForm.handleSubmit(onForgotSubmit)} className="space-y-4">
                  ...
                  <button
                    ...
                    className="btn-primary w-full py-3 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {forgotLoading ? <Loader2 size={18} className="animate-spin" /> : <>Send Reset Link <ArrowRight size={16} /></>}
                  </button>
                </form>
              </>
            )}
